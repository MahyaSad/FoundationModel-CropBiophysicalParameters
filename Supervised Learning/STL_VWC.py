...
class SupervisedModel(nn.Module):
    def __init__(self, pre_trained_vit_SAR,pre_trained_vit_Opt, input_dim, hidden_dim, output_dim_vwc, output_dim_height, dropout):
        ...
        self.relu = nn.ReLU()
        self.conv1d = nn.Conv1d(13, hidden_dim, kernel_size=3, padding=1)  # Assume input_dim is the depth of feature channels
    ...
    def forward(self, x):
        ...
        features = torch.cat((additional_features[:,:,0:1],additional_features[:,:,3:6],additional_features[:,:,6:9]), axis=-1)
...
best_model.load_state_dict(torch.load('VH_S2_climate.pth'))
best_model.eval()
