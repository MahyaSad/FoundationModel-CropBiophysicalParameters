...
class CROMA(nn.Module):
    ...
    def forward(self, imgs, radar_mask_info):
        ...
        #print(patchified_radar_imgs.shape)
...
class SupervisedModel(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            #fused_features = radar_GAP + Opt_GAP
        ...
        features = torch.cat((additional_features[:,:,3:6],additional_features[:,:,6:9]), axis=-1)
        ...
        # Optional: additional convolutional or pooling layers can be added here
        ...
        output_height = self.output_projection_height(x)
        
        return output_vwc, output_height
...
def compute_loss(output_vwc, labels_vwc, output_height, labels_height, criterion):
    loss_vwc = criterion(output_vwc, labels_vwc)
    loss_height = criterion(output_height, labels_height)
    return loss_vwc + loss_height  # Simple sum of losses
...
test_labels[:, 0] = vwc_scaler.transform(test_labels[:, 0].reshape(-1, 1)).flatten()
...
best_model.load_state_dict(torch.load('S2_climate.pth'))
best_model.eval()
test_predictions_vwc, test_actuals_vwc = [], []
test_predictions_height, test_actuals_height = [], []
...
with torch.no_grad():
        for X_test, y_test in test_loader:
            ...
            for i in range(y_test.size(0)):
                ...
                y_vwc_test, y_height_test = Y_test[:, :, 0].to(device), Y_test[:, :, 1].to(device)
                output_vwc_test, output_height_test = best_model(X_test[i:i+1, :, :, :, :])
                test_predictions_vwc.extend(output_vwc_test[:, :, 0].cpu().numpy().flatten())
                test_actuals_vwc.extend(y_vwc_test.cpu().numpy().flatten())
                test_predictions_height.extend(output_height_test[:, :, 0].cpu().numpy().flatten())
                test_actuals_height.extend(y_height_test.cpu().numpy().flatten())
mae_vwc, rmse_vwc, r2_vwc = calculate_metrics(np.array(test_actuals_vwc), np.array(test_predictions_vwc), vwc_scaler)   

mae_height, rmse_height, r2_height = calculate_metrics(np.array(test_actuals_height), np.array(test_predictions_height), height_scaler)     


print(f"Test Results:")
print(f"VWC - MAE: {mae_height:.4f}, RMSE: {rmse_height:.4f}, R^2: {r2_height:.4f}")
