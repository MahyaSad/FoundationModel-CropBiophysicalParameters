...
class DecoderMAE(nn.Module):
    ...
    def forward(self, x, mask_info_radar, target):
        ...
        num_patches_per_dim = int((256 / self.patch_size))  # This should be 32
        ...
        pred_radar = rearrange(x, 'b (h w) (c ph pw) -> b c (h ph) (w pw)', 
                               ...
                               ph=self.patch_size, pw=self.patch_size, c=3)
        

        # Apply patch-wise normalization to the target
        mean = target.mean(dim=-1, keepdim=True)
        var = target.var(dim=-1, keepdim=True)
        target_normalized = (target - mean) / (var + 1.e-6) ** .5
        # Rearrange the target to match pred_radar
        target_radar = rearrange(target_normalized, 'b (h w) (c ph pw) -> b c (h ph) (w pw)', 
                                 h=num_patches_per_dim, w=num_patches_per_dim, 
                                 ph=self.patch_size, pw=self.patch_size, c=3)

        # Compute the radar loss
        ...
        loss_radar = (loss_radar * mask).sum() / (mask.sum() + 1e-8)  # Adding small epsilon to avoid division by zero
...
optimizer = torch.optim.Adam(model.parameters(), lr=6.777439597940801e-05)  # Example optimizer and learning rate
...
means, stds = compute_channel_stats(stacked_patches)
...
# Select 3 random patches for visualization
np.random.seed(42)
random_patch_indices = np.random.choice(len(dataset), 3, replace=False)
random_patches = [dataset[i].numpy() for i in random_patch_indices]
...
mask_ratio = 0.136  
test_dataset = PatchesDataset(normalized_data)  # Assuming you have a separate test set
